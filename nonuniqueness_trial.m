...
% load mesh
mesh = toastMesh('adaptive.msh','gmsh');
...
basis1 = toastBasis(mesh, grd2);
mua = basis1.Map('B->M', bmua);
mus = basis1.Map('B->M', bmus);

mesh_f = toastMesh('adaptive.msh','gmsh');
    basisf = toastBasis(mesh_f,grd2);
    ...
    data_model_f = (log(mvecf'*(sysMatf\Qf)));
...
mua_hmg = ones(nv,1)*mua_bkg;
mus_hmg = ones(nv,1)*mus_bkg;
smat = dotSysmat(mesh, mua, mus_hmg, ref);
