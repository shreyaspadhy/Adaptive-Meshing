function pde_constraint


disp('PDE-Constrained Optimization')
disp('2D image reconstruction with Gauss-Newton solution of PDE-Constrained system')
disp('-------------------------------------------------------')
...
% ======================================================================
verbosity = 1;
meshdir = '/home/spadhy/Desktop/toastpp/test/2D/meshes/';       % example mesh repository
qmname  = [meshdir 'circle25_1x1.qm']; % source-detector definitions
...
tolCG = 1e-15;                           % Gauss-Newton convergence criterion
...
%DOMAIN AND OBJECT SETTINGS DONE
...
%Data stores log(measured) in real+imag form
...
hbasis = toastBasis(hmesh,grd);         %Maps b/w hmesh and grd
...
hmeshinv = toastMesh (invmesh);                        % read inverse solver mesh
hmeshinv.ReadQM (qmname);                              % add source/detector descriptions
ninv = hmeshinv.NodeCount ();                            % number of nodes
dmask = hmeshinv.DataLinkList ();                      % source-detector connectivity

% Set up homogeneous initial parameter estimates
hbasis = toastBasis (hmeshinv, grd, 'LINEAR');         % maps between mesh and reconstruction basis

mua = ones(ninv,1) * 0.025;                            % initial mua estimate
%mus = ones(n,1) * 2;                                % initial mus estimate
mus = hbasis.Map ('B->M', mustgt_img);
ref = ones(ninv,1) * refind;                           % refractive index estimate
...
qvec = hmeshinv.Qvec ('Neumann', 'Gaussian', 2);       % nodal source vectors

% Generate measurement vectors
mvec = hmeshinv.Mvec ('Gaussian', 2, ref);             % nodal measurement vectors

% Initial data set f[x0] with homogenous parameter guess
smat = dotSysmat (hmeshinv, mua, mus, ref, freq);      % FEM system matrix
phi = full(smat\qvec);
lgamma = reshape (log(mvec.' * (phi)), [], 1);% solve for photon density and map to boundary measurements
lgamma = lgamma(dmask);                             % remove unused source-detector combinations
...
% solution vector
x = [scmua;sckap];                                  % linear solution vector
%x = scmua;
...
size(lambda)
size(smat)
delLdelphi = mvec*(y - mvec.'*phi) + smat.'*lambda;

delLdellambda = phi.'*(smat.') - (qvec.');
...
%                        data, sd, 'method', 'gmres', 'tolerance', 1e-12);
%     r(size(r)/2+1:size(r)) = 0;
...
%     imagesc(mus_img% while (itr <= itrmax) && (err > tolCG*err0) && (errp-err > tolCG)
% 
%     errp = err;
% 
%     % Gradient of cost function
...
%     set(gca,'Position',[0.01 0.05 0.4 0.4]);
% 
%     proj = toastProject (hmeshinv, mua, mus, ref, freq, qvec, mvec);
% 
